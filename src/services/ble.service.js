...
export default class BleService extends EventEmitter {
  ...
  static EFFECT_PROPERTY_CHARACTERISTIC =
    "19B10004-E8F2-537E-4F6C-D104768A1214";
  static PATTERN_COMPLETE_CHARACTERISTIC =
  ...
  async scan(resultCallback, stopCallback) {
    ...
    await BleClient.requestLEScan(
      {
        services: [BleService.LED_SERVICE],
      },
      resultCallback
    );
  ...
  async setColor(strip, hue, saturation, brightness) {
    if (this.device) {
      ...
      await BleClient.write(
        ...
        BleService.COLOR_CHARACTERISTIC,
        dataView
      );
    }
  }
  ...
  async startPattern(strip) {
    ...
    await BleClient.write(
      this.device.deviceId,
      BleService.LED_SERVICE,
      BleService.PATTERN_UPLOAD_CHARACTERISTIC,
      dataView
    );
  }
  ...
  async sendEffect(id) {
    ...
    await BleClient.write(
      this.device.deviceId,
      BleService.LED_SERVICE,
      BleService.EFFECT_CHARACTERISTIC,
      dataView
    );
  }
  ...
  async sendProperty(id, value) {
    ...
    await BleClient.write(
      this.device.deviceId,
      BleService.LED_SERVICE,
      BleService.EFFECT_PROPERTY_CHARACTERISTIC,
      dataView
    );
  }
  ...
  async endPattern() {
    ...
    await BleClient.write(
      this.device.deviceId,
      BleService.LED_SERVICE,
      BleService.PATTERN_COMPLETE_CHARACTERISTIC,
      dataView
    );
  }
  ...
  async connect(device, options) {
    ...
    await BleClient.connect(
      device.deviceId,
      this.onDisconnect.bind(this, device),
      options
    );
    this.device = device;
